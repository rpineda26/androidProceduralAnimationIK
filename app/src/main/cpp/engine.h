//
// Created by Ralph Pineda on 2/25/25.
//

#ifndef VULKANANDROID_ENGINE_H
#define VULKANANDROID_ENGINE_H
...
//        void createDevice();
    ...
    std::vector<Vertex> HelloVK::createCubeVertices() {
        return {
                // Front face
                ...
                {{-0.5f, 0.5f, 0.5f}, {1.0f, 1.0f, 1.0f}, {0.0f, 1.0f}},   // 3: front-top-left, white

                // Back face
                {{-0.5f, -0.5f, -0.5f}, {1.0f, 1.0f, 0.0f}, {1.0f, 0.0f}}, // 4: back-bottom-left, yellow
                {{0.5f, -0.5f, -0.5f}, {1.0f, 0.0f, 1.0f}, {0.0f, 0.0f}},  // 5: back-bottom-right, magenta
                {{0.5f, 0.5f, -0.5f}, {0.0f, 1.0f, 1.0f}, {0.0f, 1.0f}},   // 6: back-top-right, cyan
                {{-0.5f, 0.5f, -0.5f}, {0.5f, 0.5f, 0.5f}, {1.0f, 1.0f}}   // 7: back-top-left, gray
        };
    }

    std::vector<uint16_t> HelloVK::createCubeIndices() {
        return {
                // Front face
                0, 1, 2, 2, 3, 0,
                // Right face
    ...
    void HelloVK::createVertexBuffer() {
        ...
        // Create a staging buffer first (for transfer)
        VkBuffer tempStagingBuffer;
        VkDeviceMemory stagingBufferMemory;
        createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                     VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                     tempStagingBuffer, stagingBufferMemory);
        ...
        // Create the actual vertex buffer (on GPU)
        ...
        vkDestroyBuffer(device, tempStagingBuffer, nullptr);
        vkFreeMemory(device, stagingBufferMemory, nullptr);
    }

    void HelloVK::createIndexBuffer() {
        std::vector<uint16_t> indices = createCubeIndices();
        ...
        // Create the actual index buffer (on GPU)
        createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                     VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, indexBuffer, indexBufferMemory);

        // Copy data from staging buffer to index buffer
        copyBuffer(tempStagingBuffer, indexBuffer, bufferSize);
 ...
 */
    ...
    void HelloVK::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        vkCmdDrawIndexed(commandBuffer, 36, 1, 0, 0, 0); // 36 indices for a cube (6 faces * 2 triangles * 3 vertices)
...
// END DEVICE SUITABILITY
    ...
    VkFormat HelloVK::findDepthFormat() {
        // List of depth formats to check (from highest to lowest quality)
        ...
        // Loop through and find a format that supports depth attachment
    ...
    void HelloVK::createRenderPass() {
        ...
        depthAttachment.format = findDepthFormat(); // You'll need to implement this
        ...
        depthAttachmentRef.attachment = 1; // Index 1 after the color attachment
...
// UV attribute
    ...
    void HelloVK::createDepthResources(){
        ...
        for(int i =0;i <depthImages.size();i++){
            ...
            VK_CHECK(vkCreateImage(device, &imageInfo, nullptr, &depthImages[i]));
            ...
            VK_CHECK(vkAllocateMemory(device,&allocInfo,nullptr, &depthImagesMemory[i]));
            VK_CHECK(vkBindImageMemory(device, depthImages[i], depthImagesMemory[i], 0));
            ...
            VK_CHECK(vkCreateImageView(device, &viewInfo, nullptr, &depthImageViews[i]));
