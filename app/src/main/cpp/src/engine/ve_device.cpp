...
void VeDevice::pickPhysicalDevice() {
  ...
  LOGI("physical device: %s", properties.deviceName);
  LOGI("Max samplers per stage: %d", properties.limits.maxPerStageDescriptorSamplers);
  LOGI("Max push constant size: %d bytes", properties.limits.maxPushConstantsSize);
}
...
void VeDevice::resetWindow() { createSurface(); }
...
void VeDevice::hasGflwRequiredInstanceExtensions() {
    ...
    for (const auto &extension : extensions) {
        LOGI("Available extensions: %s", extension.extensionName);
    ...
    for (const auto &optional : optionalExtensions) {
        if (available.find(optional) != available.end()) {
            LOGI("\t%s is supported.", optional);
        } else {
            LOGI("\t%s is not supported.", optional);
        }
    }
}
