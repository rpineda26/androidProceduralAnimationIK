...
void VeSwapChain::createSwapChain() {
  ...
  determineOrientation(swapChainSupport.capabilities);
...
VkExtent2D VeSwapChain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
    if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
        ...
        //alternative solution: from context of resizable windows
        ...
        //alternative solution based on rotating orientation in android environment
    ...
    void VeSwapChain::determineOrientation(const VkSurfaceCapabilitiesKHR& capabilities) {
        // Check current transform flags
        ...
        if (transform & VK_SURFACE_TRANSFORM_ROTATE_90_BIT_KHR) {
            ...
            orientation = Orientation::PORTRAIT_REVERSED;
        }

        // Default to portrait if no rotation is specified
